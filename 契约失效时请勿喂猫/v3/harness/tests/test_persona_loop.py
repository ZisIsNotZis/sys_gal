...
class PersonaLoopTests(unittest.TestCase):
    def test_seed_personas_include_social_brakes(self):
        ...
        amani = seeds["amani-njoroge"].identity + seeds["amani-njoroge"].private_seed
        self.assertIn("does not repeat herself", amani)
        lin = seeds["lin-yao"].identity + seeds["lin-yao"].private_seed
        self.assertIn("rather pause than pester", lin)
    ...
    def test_answerer_persona_stops_after_first_clear_answer(self):
        ...
        Runner(world, {"a": a_agent, "b": b_agent}, states, trace, max_workers=2).run(
            stop_at=world.now + timedelta(minutes=1), max_turns=40)
