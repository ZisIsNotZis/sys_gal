...
class WorldTests(unittest.TestCase):
    ...
    def test_move_rejection_explains_closure_and_open_neighbors(self):
        ...
        with self.assertRaises(ActionRejected) as ctx:
            w.submit(Intention("a", "move", {"target": "office", "duration_seconds": 60}, w.version))
        self.assertIn("closed", str(ctx.exception))
        with self.assertRaises(ActionRejected) as ctx:
            w.submit(Intention("a", "move", {"target": "far", "duration_seconds": 60}, w.version))
        self.assertIn("not a known place", str(ctx.exception))
    ...
    def test_open_close_requires_controllable(self):
        ...
        self.assertIn("not under your control", str(ctx.exception))
    ...
    def test_prompt_describes_physical_interaction_limits(self):
        ...
        self.assertIn("interact (target=office, verb=knock, parameters={})", text)
