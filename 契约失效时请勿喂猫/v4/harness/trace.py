...
class Trace:
    def __init__(self, seed_version: str, run_id: str) -> None:
        ...
        # v4: parameter-name alias hits (V4-DESIGN §2 telemetry) and session
    ...
    def record_alias_telemetry(self) -> dict[str, int]:
        ...
        from . import adapter
        current = adapter.alias_telemetry()
        for key, count in current.items():
            self.alias_telemetry[key] = count
        return current

    def record_compaction(self, actor: str, turn_id: str | None) -> None:
        """Record where one actor's live context was rewritten (§5.3)."""
        self.compactions.append({"actor": actor, "turn_id": turn_id})
    ...
    def record_agent(self, *, state: PrivateState, perception: dict[str, Any],
        ...
        self.agent_turns.append({
            ...
            # 心声：the actor's private monologue; never part of world events.
            "inner": getattr(intention, "inner", None), "private": True,
            # V4-CAST §1: mc | npc | extra — analysis marker, never shown to MCs.
            "role": role,
